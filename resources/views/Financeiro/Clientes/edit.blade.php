...
@php
    $permissaoEfi     = ($cliente['checkbox_efi'] ?? 0) == 1;
    $permissaoPagbank = ($cliente['checkbox_pagbank'] ?? 0) == 1;
@endphp
...
<div class="cadastro-cliente-page">
    ...
    <div class="cadastro-cliente-header">
        <div>
            <span class="cadastro-cliente-eyebrow">Cliente #{{ $cliente['id_cliente'] }}</span>
            <h1 class="cadastro-cliente-title">Editar cliente</h1>
            <p class="cadastro-cliente-subtitle">
                Atualize os dados do cliente, as credenciais de cobrança e os produtos de estoque vinculados.
            </p>
        </div>
    </div>

    <form action="{{ route('financeiro-clientes-atualizar', $cliente['id_cliente']) }}" id="cadastro-cliente-form" class="cadastro-cliente-grid needs-validation" method="post" enctype="multipart/form-data" novalidate>
        @csrf
        ...
        <div class="cadastro-cliente-form-col">
            ...
            <section class="cc-section">
                ...
                <div class="cc-section-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="cliente_nome" id="cliente_nome" value="{{ $cliente['cliente_nome'] }}" required>
                    ...
                    <div class="mb-0">
                        ...
                        <input type="text" class="form-control" name="cliente_cpf_cnpj" id="cliente_cpf_cnpj" value="{{ $cliente['cliente_cpf_cnpj'] }}" maxlength="18" required>
                        <div class="invalid-feedback">
                ...
                <div class="cc-section-body">
                    <div class="row g-3 mb-3">
                        <div class="col-sm-4">
                            ...
                            <div class="cc-cep-wrap">
                                <input type="text" class="form-control" name="cliente_cep" id="cliente_cep" value="{{ $cliente['cliente_cep'] ?? '' }}" placeholder="00000-000" maxlength="9" required>
                        ...
                        <div class="col-sm-8">
                            ...
                            <input type="text" class="form-control" name="cliente_logradouro" id="cliente_logradouro" value="{{ $cliente['cliente_logradouro'] ?? '' }}" placeholder="Rua, avenida..." required>
                            <div class="invalid-feedback">
                                <p class="invalid-p">Campo obrigatório</p>
                            </div>
                        </div>
                    ...
                    <div class="row g-3 mb-0">
                        ...
                        <div class="col-sm-2">
                            ...
                            <input type="text" class="form-control text-uppercase" name="cliente_uf" id="cliente_uf" value="{{ $cliente['cliente_uf'] ?? '' }}" maxlength="2" required>
                            <div class="invalid-feedback">
            ...
            <section class="cc-section">
                <header class="cc-section-header">
                    ...
                    <div>
                        ...
                        <span class="cc-section-title">Contato</span>
                    </div>
                </header>
                <div class="cc-section-body">
                    <div class="row g-3 mb-0">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" name="cliente_celular" id="cliente_celular" value="{{ $cliente['cliente_celular'] }}" placeholder="(00) 00000-0000" required>
                            <div class="invalid-feedback">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="email" class="form-control" name="cliente_email" id="cliente_email" value="{{ $cliente['cliente_email'] }}" placeholder="contato@empresa.com.br" required>
                            <div class="invalid-feedback">
                    ...
                    <div class="cc-gateway-row" id="row-efi">
                        <div class="cc-gateway-info">
                            ...
                            <div>
                                ...
                                <p id="label-efi" class="cc-gateway-status">{{ $permissaoEfi ? 'Vinculado' : 'Não vinculado' }}</p>
                            </div>
                        </div>
                        <div class="form-check form-switch mb-0">
                            <input class="form-check-input" name="checkbox_efi" type="checkbox" role="switch" id="checkboxEfi" {{ $permissaoEfi ? 'checked' : '' }}>
                        </div>
                    </div>
                    ...
                    <div class="cc-gateway-row" id="row-pagbank">
                        <div class="cc-gateway-info">
                            ...
                            <div>
                                ...
                                <p id="label-pagbank" class="cc-gateway-status">{{ $permissaoPagbank ? 'Vinculado' : 'Não vinculado' }}</p>
                            </div>
                        </div>
                        <div class="form-check form-switch mb-0">
                            <input class="form-check-input" name="checkbox_pagbank" type="checkbox" role="switch" id="checkboxPagbank" {{ $permissaoPagbank ? 'checked' : '' }}>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="cc-section">
                ...
                <div class="cc-section-body">
                    <p class="text-muted small mb-2">Produtos vinculados atualmente. Remova os que não fazem mais parte do plano do cliente.</p>

                    <div id="vinculados-lista" class="cc-produtos-lista mb-3">
                        @forelse($produtosVinculados as $v)
                            <div class="produto-linha" data-subtotal="{{ (float) ($v['quantidade'] ?? 0) * (float) ($v['produto']['valor'] ?? 0) }}">
                                <div class="produto-linha-info">
                                    <strong>{{ $v['produto']['nome_produto'] ?? 'Produto removido' }}</strong>
                                    <span>{{ $v['quantidade'] }} un. · R$ {{ number_format(($v['quantidade'] ?? 0) * ($v['produto']['valor'] ?? 0), 2, ',', '.') }}</span>
                                </div>
                                <button type="button" class="produto-linha-remover vinculado-remover" data-id-vinculo="{{ $v['id'] }}" aria-label="Remover {{ $v['produto']['nome_produto'] ?? 'produto' }}">
                                    <iconify-icon icon="solar:trash-bin-trash-bold-duotone"></iconify-icon>
                                </button>
                            </div>
                        @empty
                            <p class="text-muted small mb-0" id="vinculados-vazio">Nenhum produto vinculado a este cliente.</p>
                        @endforelse
                    </div>
                    <div id="produtos-removidos-inputs"></div>

                    <hr class="my-3">

                    <p class="text-muted small mb-3">Adicionar novos produtos. A quantidade é baixada do estoque assim que o cliente for salvo.</p>
                    ...
                    @else
                        ...
                        <div id="produtos-lista" class="cc-produtos-lista">
                            <p class="text-muted small mb-0" id="produtos-vazio">Nenhum produto novo adicionado ainda.</p>
                        </div>
            ...
            <div class="cc-submit-row">
                <a href="{{ route('financeiro-clientes') }}" class="btn btn-outline-secondary">Cancelar</a>
...
<script>
    $(document).ready(function () {
        ...
        updateGatewayRow('row-efi', 'icon-efi', 'label-efi', $('#checkboxEfi').is(':checked'), '22,163,74');
        updateGatewayRow('row-pagbank', 'icon-pagbank', 'label-pagbank', $('#checkboxPagbank').is(':checked'), '37,99,235');

        // ---- Produtos já vinculados (remover) ----
        $(document).on('click', '.vinculado-remover', function () {
            var $row = $(this).closest('.produto-linha');
            var idVinculo = $(this).data('id-vinculo');

            $('#produtos-removidos-inputs').append('<input type="hidden" name="produtos_removidos[]" value="' + idVinculo + '">');
            $row.remove();

            if ($('#vinculados-lista .produto-linha').length === 0) {
                $('#vinculados-lista').html('<p class="text-muted small mb-0" id="vinculados-vazio">Nenhum produto vinculado a este cliente.</p>');
            }
        });

        // ---- Novos produtos do estoque ----
        ...
        function renderProdutos() {
            ...
            if (produtosAdicionados.length === 0) {
                $lista.html('<p class="text-muted small mb-0" id="produtos-vazio">Nenhum produto novo adicionado ainda.</p>');
            } else {
                var html = '';
        ...
        $(document).on('click', '#produtos-lista .produto-linha-remover', function () {
