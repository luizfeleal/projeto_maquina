<section id="relatorios" class="dash-section">
    ...
    <div class="dash-card" style="overflow:hidden;">
        ...
        <div id="relatorios-totalTransacoes" class="rel-panel" style="padding:24px;">
            <form action="{{ route('relatorio-criar') }}" method="post">
                @csrf
                <input type="hidden" name="tipo" value="totalTransacoes">
                <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(220px,1fr)); gap:16px;">
                    <div style="grid-column:1/-1;">
                        <label class="form-label" style="font-size:.75rem; font-weight:600;">Cliente</label>
                        <select class="select-cliente-transacoes form-control" name="id_cliente[]" multiple>
        ...
        <div id="relatorios-taxasDesconto" class="rel-panel" style="padding:24px; display:none;">
            <form action="{{ route('relatorio-criar') }}" method="post">
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(220px,1fr)); gap:16px;">
                    <div style="grid-column:1/-1;">
                        <label class="form-label" style="font-size:.75rem; font-weight:600;">Cliente</label>
                        <select class="select-cliente form-control" name="id_cliente[]" multiple>
                            @foreach($clientes as $cliente)
                                <option value="{{ $cliente['id_cliente'] }}">{{ $cliente['cliente_nome'] }}</option>
                            @endforeach
                        </select>
        ...
        <div id="relatorios-relatorioErros" class="rel-panel" style="padding:24px; display:none;">
            <form action="{{ route('relatorio-criar') }}" method="post">
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(220px,1fr)); gap:16px;">
                    <div>
                        <label class="form-label" style="font-size:.75rem; font-weight:600;">Cliente</label>
                        <select class="select-cliente-erros form-control" name="id_cliente[]" multiple>
                            @foreach($clientes as $cliente)
                                <option value="{{ $cliente['id_cliente'] }}">{{ $cliente['cliente_nome'] }}</option>
                            @endforeach
                        </select>
        ...
        <div id="relatorios-maquinasOnOff" class="rel-panel" style="padding:24px; display:none;">
            <p style="margin:0 0 16px; font-size:.85rem; color:var(--sp-muted);">
                Gera um relatório com o status atual de todas as máquinas (online e offline).
            </p>
            <form action="{{ route('relatorio-criar') }}" method="post">
                @csrf
