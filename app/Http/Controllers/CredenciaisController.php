...
class CredenciaisController extends Controller
{
    ...
    public function listarCredenciais(Request $request){
        ...
        if($id_cliente){
            $credenciais = array_filter($credenciais, fn($c) => ($c['id_cliente'] ?? null) == $id_cliente);
        }
        if($tipo_cred){
    ...
    public function registrarCredencial(Request $request){
        ...
        try{
            ...
            if($result['success'] != true){
                ...
                \Log::error('Erro ao cadastrar credencial (painel admin): ' . json_encode($result['error'] ?? $result));
            ...
            \Log::error('Exceção ao cadastrar credencial (painel admin): ' . $e->getMessage());
    ...
    public function editarCredencialPagbank(Request $request, $id){
        ...
        $credencial = (array) $credencial;
        $credencial['id'] = $credencial['id_cred_api_pix'] ?? $credencial['id'] ?? $id;
        if(($credencial['tipo_cred'] ?? '') !== 'pagbank'){
            return redirect()->route('credencial-listar')->with('error', 'Credencial não encontrada ou tipo incorreto para esta rota.');
        }
    ...
    public function atualizarCredencial(Request $request, $id){
        try{
            ...
            if($result['success'] != true){
                ...
                \Log::error('Erro ao atualizar credencial (painel admin, id=' . $id . '): ' . json_encode($result['error'] ?? $result));
                return back()->with('error', $mensagem);
            }else{
                return back()->with('success', $result['data']['message']);
            }
        }catch(\Throwable $e){
            \Log::error('Exceção ao atualizar credencial (painel admin, id=' . $id . '): ' . $e->getMessage());
