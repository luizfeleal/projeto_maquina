...
class QrCodeController extends Controller
{
    ...
    public function registrarQr(Request $request)
    {
        ...
        try {
            ...
            if($qr->successful()){
                $body = $qr->json();
                return back()-> with(['success' => $body['message'] ?? 'QR Code cadastrado com sucesso!', 'qr_base64_imagem' => $body['response']['qr_image'] ?? null, 'id_local' => $request['select_local'], 'id_maquina' =>$request['select_maquina']]);
            }
